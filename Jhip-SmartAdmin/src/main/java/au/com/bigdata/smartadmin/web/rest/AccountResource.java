...
import au.com.bigdata.smartadmin.web.rest.dto.KeyAndPasswordDTO;
...
public class AccountResource {
    ...
    public ResponseEntity<?> registerAccount(@Valid @RequestBody UserDTO userDTO, HttpServletRequest request) {
        return userRepository.findOneByLogin(userDTO.getLogin())
            ...
            .orElseGet(() -> userRepository.findOneByEmailAddress(userDTO.getEmailAddress())
                .map(user -> new ResponseEntity<>("e-mail address already in use", HttpStatus.BAD_REQUEST))
                .orElseGet(() -> {
                    User user = userService.createUserInformation(userDTO.getLogin(), userDTO.getPassword(),
                    userDTO.getFirstName(), userDTO.getLastName(), userDTO.getEmailAddress().toLowerCase(),
                    userDTO.getLangKey());
    ...
    @Timed
    @RequestMapping(value = "/users/userAccountActivationById/{login}", method = RequestMethod.PUT, produces = MediaType.APPLICATION_JSON_VALUE)
    @Secured(AuthoritiesConstants.ADMIN)
    public ResponseEntity<Void> accountActivationById(@PathVariable String login) {
    	 log.debug("REST request to activate User: {}", login);
         userRepository.findOneById(Long.parseLong(login))
         .map(user -> {
         ...
         return ResponseEntity.ok().headers(HeaderUtil.createAlert( "userManagement.activated", login)).build();
     }
    
    
    @Timed
    @RequestMapping(value = "/users/userAccountLockById/{login}", method = RequestMethod.PUT, produces = MediaType.APPLICATION_JSON_VALUE)
    @Secured(AuthoritiesConstants.ADMIN)
    public ResponseEntity<Void> userAccountLockById(@PathVariable String login) {
    	 log.debug("REST request to activate User: {}", login);
         userRepository.findOneById(Long.parseLong(login))
         .map(user -> {
             // activate given user for the registration key.
             ...
             userRepository.save(user);
             userSearchRepository.save(user);
             log.debug("De-Activated user: {}", user);
             return user;
         });
    ...
    public ResponseEntity<String> saveAccount(@RequestBody UserDTO userDTO) {
        ...
        return userRepository
            ...
            .map(u -> {
                userService.updateUserInformation(userDTO.getFirstName(), userDTO.getLastName(), userDTO.getEmailAddress(),
                    userDTO.getLangKey());
